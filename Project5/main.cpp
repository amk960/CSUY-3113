...
void Initialize() {
    ...
    hit = Mix_LoadWAV("success.wav");
    kill = Mix_LoadWAV("failure.wav");
...
void Update() {
    ...
    //currentScene->state.player->CheckCollisionsY(currentScene->state.enemies, currentScene->LEVEL1);
    ...
    if (currentScene->state.player->collidedEnemy != NULL) {
        for (int i = 0; i < currentScene->state.enemy_count; i++) {
            ...
            if (currentScene->state.player->collidedEnemy == &(currentScene->state.enemies[i]) && (currentScene->state.player->killEnemy== true) && (currentScene->state.player->hitEnemy == false)) {
                Mix_PlayChannel(-1, kill, 0);
                currentScene->state.player->killEnemy = false;
                currentScene->state.player->hitEnemy = false;
                currentScene->state.enemies[i].position = glm::vec3(10.0f, 10.0f, 0.0);
                Entity* new_enemies = new Entity[currentScene->state.enemy_count - 1];
                int l=0;
                for (int k = 0; k < currentScene->state.enemy_count-1; k++) {
                    if (&(currentScene->state.enemies[l]) == &(currentScene->state.enemies[i])) {
                        l++;
                    }
                    new_enemies[k] = currentScene->state.enemies[l];
                    
                    l++;
                }
                currentScene->state.enemy_count -=1;
                delete[] currentScene->state.enemies;
                currentScene->state.enemies = new_enemies;

                currentScene->state.player->collidedEnemy = NULL;
                break;
            }
            else if (currentScene->state.player->collidedEnemy == &(currentScene->state.enemies[i]) && (currentScene->state.player->hitEnemy == true) ) {
